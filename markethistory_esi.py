...
def get_data(session, type_id, region_id):
    ...
    future = session.get(url, params=payload, timeout=30.5)
...
if __name__ == '__main__':
    ...
    market_types = types[(types.published == 1) & (types.marketGroupID != 'None')].typeID
    ...
    with gzip.open('latest-history.csv.gz', 'w') as f:
        ...
        for region_id in regions.regionID:
            futures = [get_data(session, type_id, region_id) for type_id in market_types]
            retryfutures = []

            for result in tqdm(cf.as_completed(futures),desc="Downloading region {}".format(region_id),total=len(futures), leave=True):
